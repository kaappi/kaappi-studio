...
| `setTheme(theme)` | both | `"light"` or `"dark"`; switches the `theme-*` body class in `index.html` **and** reconfigures the CodeMirror caret color, dark flag, and syntax-highlight palette through `editor.js`'s `setTheme` (a `Compartment` reconfigure, so the document, selection, scroll position and undo history survive). A call that arrives before `ready` only sets the body class; the editor reads it when it is created, so no theme flash. |
